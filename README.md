...
- 前面サービス（常駐通知）として動くため、アプリを閉じても（ホームに戻る/他アプリ切替）記録は続く。
- **スワイプでタスクを消しても復帰**（`onTaskRemoved` で再起動を予約）、**端末再起動後も自動で記録再開**（`BootReceiver`。記録中フラグを保存しておき、`BOOT_COMPLETED` で再開）。
  - ただし Android 12+ の背景 FGS 起動制限や OEM の省電力管理により、機種によっては再開が抑止されることがある（ベストエフォート）。

### 背景で確実に記録し続けるための端末設定

1. 位置情報を **「常に許可」**（設定 → アプリ → GPS Tracker → 権限 → 位置情報）。無いと画面を消した瞬間に測位できず「位置不明」レコードばかりになる。
2. **電池の最適化を無効化 / 制限なし**（設定 → アプリ → GPS Tracker → 電池）。OEM の省電力が背景サービスを間引く/停止するのを防ぐ。
